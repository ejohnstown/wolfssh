...
static void blf_init(blf_ctx *bc);
static void blf_key_cleanup(blf_ctx *bc);
static void blf_enc(blf_ctx *bc, word32 *data, int blocks);

static word32 F(blf_ctx *bc, word32 x)
{
    word16 a;
    word16 b;
    word16 c;
...
static void Blowfish_encipher(blf_ctx *bc, word32 *xl, word32 *xr)
{
    word32 Xl;
    word32 Xr;
    word32 temp;
...
static void Blowfish_decipher(blf_ctx *bc, word32 *xl, word32 *xr)
{
    word32 Xl;
    word32 Xr;
    word32 temp;
...
static void blf_key_init(blf_ctx *bc, byte *key, int len)
{
    Blowfish_initialize(bc);
    Blowfish_setkey(bc, key, len);
}

static void blf_dec(blf_ctx *bc, word32 *data, int blocks)
{
    word32 *d;
    int i;
    ...
    d = data;
    for (i = 0; i < blocks; i++) {
        Blowfish_decipher(bc, d, d+1);
        d += 2;
    }
}

void wolfSSH_TestBlfInit(blf_ctx *bc)
{
    blf_init(bc);
}

void wolfSSH_TestBlfKeyInit(blf_ctx *bc, byte *key, int len)
{
    blf_key_init(bc, key, len);
}

void wolfSSH_TestBlfKeyCleanup(blf_ctx *bc)
{
    blf_key_cleanup(bc);
}

void wolfSSH_TestBlfEnc(blf_ctx *bc, word32 *data, int blocks)
{
    blf_enc(bc, data, blocks);
}

void wolfSSH_TestBlfDec(blf_ctx *bc, word32 *data, int blocks)
{
    blf_dec(bc, data, blocks);
}

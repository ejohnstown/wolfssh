...
typedef int (*WS_CallbackChannelReqAny)(WOLFSSH_CHANNEL* channel,
        const char* type, word32 typeSz, const byte* data, word32 dataSz,
        void* ctx);
 ...
 * above that answers the rest. name is the request name as it arrived,
 * nameSz bytes, and data is the request's type-specific part, dataSz
 * bytes, for the callback to parse, so a tcpip-forward can be set up from
 * here without a forward callback. Neither is NUL terminated, and a name
 * may hold any byte, so a policy matches on nameSz bytes rather than with
 * the string functions. UNHANDLED leaves the request to those callbacks.

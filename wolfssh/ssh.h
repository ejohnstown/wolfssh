 ...
 * receiving direction open (the half-close of RFC 4254 section 5.3). Sends on
 * the channel then fail with WS_EOF; reads work until the peer sends its own
 * EOF or closes. Idempotent: a second call puts no second EOF on the wire.
 *
 ...
 *
 * Returns WS_SUCCESS, WS_BAD_ARGUMENT on a NULL channel, or a send-path
 * status such as WS_WANT_WRITE. */

...
/* The following test vectors come from Schneier's RFC draft. */
static const BlowfishTestVector blowfishTestVectors[] = {
...
{
    word32 i;
    word32 tc = sizeof(blowfishTestVectors)/sizeof(BlowfishTestVector);
    const BlowfishTestVector* tv = NULL;
    blf_ctx c;
    byte key[8];
    word32 value[2];
    int ret = 0;

    for (i = 0, tv = blowfishTestVectors; i < tc; i++, tv++) {

...
static int SendDisconnectAndDrain(WOLFSSH* ssh, SOCKET_T fd)
{
    ...
    while (spins++ < 100) {
        ...
        /* WS_EOF and WS_EXTDATA are the channel's, not the socket's: the
         * peer is still there, so they read like any other message. */
        if (ret == WS_SUCCESS || ret == WS_CHAN_RXD || ret == WS_EOF ||
                ret == WS_EXTDATA || ret == WS_CHANNEL_CLOSED ||
                ret == WS_REKEYING || error == WS_WANT_READ ||
                error == WS_WANT_WRITE)

...
- A peer's channel EOF is now reported to the application as `WS_EOF` rather
  than answered with an EOF of our own, so a half-closed channel can still be
  written to. `ssh host 'sort' < file` returns output again. (PR 1195)
- `wolfSSH_ChannelExit()` now leaves the channel on the session list until
  the peer answers, so the application's pointer stays valid until
  `wolfSSH_worker()` reports `WS_CHANNEL_CLOSED`. (PR 1195)
- A client now refuses a `forwarded-tcpip` open that matches no forward
  registered with `wolfSSH_FwdRemoteSetup()`. Opt out with
  `wolfSSH_SetFwdRemoteMatch(WOLFSSH_FWD_MATCH_OFF)`. (PR 1148, 1220)
- A client now refuses `tcpip-forward` and `cancel-tcpip-forward`, answering
  `REQUEST_FAILURE` where the peer asked for a reply. A registered `fwdCb`
  is no longer the only gate. (PR 1214)
- `WOLFSSH_FWD_LOCAL_CLEANUP` is now sent, once per successful
  `WOLFSSH_FWD_LOCAL_SETUP`, with the channel id in `port`. A `fwdCb` that
  frees that state elsewhere must not free it again here. (PR 1229)
- A message the connection state disallows now draws a `DISCONNECT` with
  `PROTOCOL_ERROR` rather than a silent teardown. (PR 1214)
- A client now refuses a `session` channel open outright, per RFC 4254
  section 6.1, ahead of any `channelOpenCb`. (PR 1224)
- SFTP `SETSTAT` and `FSETSTAT` now apply the attributes they acknowledge,
  or answer `SSH_FX_OP_UNSUPPORTED`, rather than `SSH_FX_OK` regardless.
  (PR 1197)
- wolfSSHd now keeps a 022 umask, so sessions create 0644 files and 0755
  directories rather than world-writable ones. `WOLFSSHD_DEFAULT_UMASK`
  sets it at build time. (PR 1269)
- `wolfSSH_shutdown()` now returns `WS_WANT_WRITE` whenever output is still
  queued, never a stale event code such as `WS_EOF`. (PR 1252)
- `wolfSSH_get_fd()` given a NULL session now returns -1, not
  `WS_BAD_ARGUMENT`. (PR 1247)
...
- Added host keys from the Windows certificate store with
  `wolfSSH_CTX_UsePrivateKey_fromStore()`, and wolfSSHd options to load host
  keys and user CAs from system stores. (PR 900)
...
- Added CI for X.509 interop, code coverage, Windows SFTP, SCP and MinGW
  tests, the API tests under TPM, a heapmath build, and weekly Espressif
  builds. (PR 989, 1158, 1058, 1165, 1241, 1038, 1255)
...
- Added tests for the channel open response, close, exec and subsystem
  callbacks, and for the default algorithm lists. (PR 1227, 1269)
- Added `wolfSSH_ChannelSendEof()` and `wolfSSH_stream_send_eof()`, the
  missing half of the RFC 4254 section 5.3 half-close: they close the sending
  direction and leave the receiving direction open. (PR 1195)
...
- Added application-driven channels: with `wolfSSH_CTX_SetAppChannels()`,
  `wolfSSH_accept()` returns after user auth and the application grants
  each session request. The echoserver's `-A` shows it. (PR 1233, 1234)
- Added `wolfSSH_CTX_SetChannelReqAnyCb()` and
  `wolfSSH_CTX_SetGlobalReqAnyCb()`, consulted first for every channel and
  global request. (PR 1236)
...
- Added `wolfSSH_AGENT_ChannelOpen()`, so an application driving its own
  channels can open the agent channel `wolfSSH_accept()` opened for it.
  (PR 1230)
- Added `wolfSSH_SCP_accept()`, so an application that binds an `scp` command
  to a channel itself can run the transfer. (PR 1231)
- Added `wolfSSH_SetFwdRemoteMatch()` to opt out of remote-forward matching.
  (PR 1220)
...
- wolfSSHd now refuses a shell, exec or subsystem request its build cannot
  serve, rather than accepting and then dropping it. (PR 1237)
- wolfSSHd now warns on options it parses but does not enforce, such as
...
- Refused the key exchange messages each side never receives, so a peer
  cannot make a server run prime-number checks on a value of its own
  choosing. (PR 1221, 1222, 1223)
- Validated custom identification strings where they are set, including the
  required CRLF terminator, rather than deep in the handshake. (PR 1218,
  1239)
- `wolfSSH_shutdown()` now flushes whatever a short send left queued, not
  just a queued disconnect. (PR 1219)
- The library now sends `WOLFSSH_FWD_LOCAL_CLEANUP`. It was part of the
  `WS_FwdCbAction` contract and was never issued, so a peer-opened forward
  leaked whatever its setup callback allocated. (PR 1229)
- Took the authorized-keys type from the wire blob, moved ML-DSA keys off
  the stack under `WOLFSSH_SMALL_STACK`, and gated ML-DSA composites on
  wolfCrypt support. (PR 1159)
- The portfwd example now registers the channel open response callbacks,
  which had no caller anywhere in the tree. (PR 1232)
- portfwd drains its buffer before leaving on end-of-input, so a transfer
  ending while the window is being credited no longer comes up short.
  (PR 1228)
...
- Fixed wolfSSHd truncating shell output under write back-pressure.
  (PR 1253, 1263)
...
- Fixed an exec of a command such as `scpbackup` reaching the built-in SCP
  server. (PR 1237)
- Fixed a fixed-port `tcpip-forward` success carrying a bound-port field.
  Thanks to the tlspuffin team. (PR 1254, resolves issue #1246)
...
- Fixed the echoserver dropping live sessions and spinning in its SFTP loop
  on a blocked write. (PR 1251, 1250)
- Fixed the Espressif echoserver's forward cleanup closing the wrong
  socket. (PR 1255)
...
- Fixed the `--disable-server` and `--disable-client` builds and their
  tests, and a `Base16_Decode` collision under `--enable-tpm`.
  (PR 1155, 1264, 1273, 1169)
- Fixed the Zephyr build on Zephyr 4.1 and newer, and against current
  wolfSSL. (PR 1267, 1265)
- Fixed the ML-DSA composite build guards, and builds without composites.
  (PR 1259, 1266)
...
- Fixed the Windows SFTP open flags, which OR-ed creation dispositions
  together instead of resolving one, and never wired up TRUNC, EXCL or
  APPEND. (PR 1173)
- Fixed wolfsshd's `-D` on Windows, which compared wide argv strings as
  narrow ones and so never matched. (PR 1173)
- Fixed a resumed SFTP `get` truncating the local file to zero on Windows,
  and a saved offset being trusted on either platform without checking the
  destination's real size. (PR 1216)
...
- Fixed an inbound `auth-agent@openssh.com` open being admitted on the
  presence of the agent object rather than on the client's request.
  (PR 1244)
- Fixed SFTP EOF appending on POSIX and short Windows writes, and stopped
  wolfsshd option parsing when a Windows argv conversion fails. (PR 1159)
